...
type FolderStorage interface {
	...
	GetAllAddress(ctx context.Context, u *dto.User) ([]*model.Folder, error)
...
func (s *folderStorage) GetAllAddress(ctx context.Context, u *dto.User) (folderModel []*model.Folder, err error) {
	q := `
		SELECT address
		FROM folder
		WHERE userId=$1
